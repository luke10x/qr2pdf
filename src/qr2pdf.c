...
cairo_image_surface_create_from_svg (const gchar* svg)
{
...
/**
 * Renders qrcode on svg of size specified in mm
 */
char *qrcode2svg(const QRcode *qrcode, int size)
{
    ...
    snprintf(
		...
		margin, margin, symwidth, symwidth, /* viewBox */
...
cairo_surface_t *
create_surface_a4_pdf(char * filename)
{
...
cairo_surface_t *
create_qr_surface(char * text, int size)
{
